...
```bash
PassoFunction$ sam build --use-container
```
...
```bash
PassoFunction$ sam logs -n StockCheckerFunction --stack-name PassoFunction --tail
```
...
```bash
PassoFunction$ pip install -r tests/requirements.txt --user
# unit test
PassoFunction$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
PassoFunction$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name PassoFunction
```
